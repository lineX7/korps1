...
class Channel:
    ...
    def new_post(self, text):
        ...
        r = requests.post(
            "https://" + self.host + "/mh/api/v24/post/create.json",
            headers = header,
    ...
    def like(self, mid, postid, likeType=1001):
        ...
        r = requests.post(
            "https://" + self.host + "/mh/api/v23/like/create.json?homeId=" + mid,
            headers = header,
            data = json.dumps(payload)
        )
    ...
    def comment(self, mid, postid, text):
        ...
        r = requests.post(
            "https://" + self.host + "/mh/api/v23/comment/create.json?homeId=" + mid,
            headers = header,
            data = json.dumps(payload)
        )
    ...
    def activity(self, limit=20):
        ...
        r = requests.get(
            "https://" + self.host + "/tl/mapi/v21/activities?postLimit=" + str(limit),
            headers = header
        )
        return r.json()
    def getAlbum(self, gid):
        ...
        r = requests.get(
            "https://" + self.host + "/mh/album/v3/albums?type=g&sourceType=TALKROOM&homeId=" + gid,
            headers = header
        )
        return r.json()
    def changeAlbumName(self,gid,name,albumId):
        ...
        r = requests.put(
            "https://" + self.host + "/mh/album/v3/album/" + albumId + "?homeId=" + gid,
            headers = header,
            data = json.dumps(payload),
        )
        return r.json()
